...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public List<SysMenu> findMenuByRoleId(Long roleId) {
        //1查询所有的菜单-添加条件status=1
        LambdaQueryWrapper<SysMenu> wrapperSysMenu = new LambdaQueryWrapper<>();
        wrapperSysMenu.eq(SysMenu::getStatus,1);
        List<SysMenu> allSysMenuList = baseMapper.selectList(wrapperSysMenu);
        //2根据角色id即roleid查询，角色菜单关系表里 角色id对应的所有的菜单id
        ...
        //3.1拿着菜单id和所有的菜单集合里面id进行比较，如果相同封装
    ...
    public List<RouterVo> findUserMenuListByUserId(Long userId) {
        ...
        //1.判断当前用户是否为管理员；如果是管理员查询可所有菜单 userId=1是管理员
        ...
        //构建成框架要求的路由结构
        List<RouterVo> routerLisr = this.buildRouter(sysMenuTreeList);
        return routerLisr;
    }
    //构建成框架要求的路由结构
    ...
    public List<String> findUserPermsByUserId(Long userId) {
        List<SysMenu> sysMenuList = null;
        //1.判断当前用户是否为管理员；如果是管理员查询可所有按钮列表 userId=1是管理员
        if(userId.longValue()==1){
            LambdaQueryWrapper<SysMenu> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(SysMenu::getStatus,1);
            sysMenuList = baseMapper.selectList(wrapper);
